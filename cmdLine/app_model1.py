    ...
    def filterDiffProj(self,proj):
        ...
        for lang in ['java', 'cxx', 'hxx']:
           ...
           for i, file_name in enumerate(os.listdir(path)):
               ...
               print "gotsome =" + str(gotsome)
               self.got_some[lang] = self.got_some[lang] or gotsome
               print "self.got_some[lang] =" + str(self.got_some[lang])
               if not ok:
    ...
    def runCCFinderSelf(self,proj,path):
        ...
        for lang in ['java', 'cxx', 'hxx']:
            if not self.got_some[lang]:
                self.progress('!! no ccFinderX executing')
                continue
