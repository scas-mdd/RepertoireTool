    ...
    def loadFromFile(self, input_path, is_rep = False):
        ...
        for line in inf:
            ...
            if reading_indices:
                ...
                if is_rep:
                    #adding following metric selection for bsd analysis:
                    #we do not need them otherwise
                    rsep = re.compile("[(:)]")
                    mv = rsep.split(metric.strip())
#                    print "metricVal = " + mv[1] + "," + mv[2]
                    if(int(mv[1].strip()) >= int(mv[2].strip())):
                        metric = mv[1].strip()
                    else:
                        metric = mv[2].strip()
